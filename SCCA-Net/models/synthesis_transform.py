...
from .CSA import Attention
from .SSFA import SSFA


class Synthesis_transform(nn.Module):

    def __init__(self, num_filters=224):
        ...
        self.b0_layer3_SFMA = SSFA(num_filters)
        ...
        self.b1_layer3_SFMA = SSFA(num_filters)
        ...
        self.b2_layer3_SFMA = SSFA(num_filters)
        ...
        self.b3_layer2 = nn.ConvTranspose2d(num_filters, 6, 3, stride=2, padding=1, output_padding=1)
    ...
    def forward(self, x):
        ...
        b0 = self.b0_layer3_SFMA(self.b0_layer3(b0))
        b0 = self.b0_layer3_igdn(b0)
        b0 += shortcut0
        ...
        b1 = self.b1_layer3_SFMA(self.b1_layer3(b1))
        b1 = self.b1_layer3_igdn(b1)
        b1 += shortcut1

        # i = 2
        attention1 = self.attention2(b1)
        b2 = self.b2_layer0_relu(self.b2_layer0(attention1))
        b2 = self.b2_layer1_relu(self.b2_layer1(b2))
        b2 += attention1
        shortcut2 = self.b2_shortcut(b2)
        b2 = self.b2_layer2_relu(self.b2_layer2(b2))
        b2 = self.b2_layer3_SFMA(self.b2_layer3(b2))
        b2 = self.b2_layer3_igdn(b2)
        b2 += shortcut2
...
if __name__ == "__main__":
    ...
    feature = torch.zeros([1,224,32,48])
